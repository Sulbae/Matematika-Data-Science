...
Original file is located at
    https://colab.research.google.com/github/Sulbae/Matematika-Data-Science/blob/main/soal-3-software-dev-ab.ipynb
...
def estimate_sample_size_proportions(p1, p2, alpha=0.05, beta=0.20, two_sided=True):
    ...
    n = np.square((np.sqrt(p_bar * q_bar * (1 + (1 / k))) * z_alpha) + (np.sqrt(p1 * q1 + ((p2 * q2) / k)) * z_beta)) / np.square(abs(p2 - p1))
    # AKHIRI KODE DI SINI
...
def generate_df_ab_test(n_days):
    ...
    user_ids_origin = create_unique_user_ids(n_origin)
    user_ids_varian = create_unique_user_ids(n_varian)
    ...
    origin_dict = {'user_id': user_ids_origin, 'user_type': 'origin', 'converted': data_origin}
    varian_dict = {'user_id': user_ids_varian, 'user_type': 'varian', 'converted': data_varian}
...
def generate_proportion_metrics(data):
  ...
  n = len(data) # Hitung banyaknya data dalam sampel (n)
  x = data.sum() # Hitung total keberhasilan (jumlah data bernilai 1)
  p = data.mean() # Hitung proporsi keberhasilan dari seluruh sampel (p)
