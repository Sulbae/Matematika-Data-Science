...
Original file is located at
    https://colab.research.google.com/github/Sulbae/Matematika-Data-Science/blob/main/soal-2-sistem-rekomendasi-ab.ipynb
...
def generate_df_ab_test(n_days=20):
  ...
  user_id_origin = create_unique_user_ids(n_origin)
  user_id_varian = create_unique_user_ids(n_varian)
...
def generate_metrics_for_t(data):
  ...
  n = len(data) # Hitung ukuran sampel (n)
  xbar = np.mean(data) # Hitung rata-rata sampel(x̄)
  std = np.std(data, ddof=1) # Hitung standar deviasi (s)
  # Simpan hasil perhitungan di atas dalam objek dataclass metrics_estimation
  metrics = metrics_estimation(
    n=n,
    xbar=xbar,
    std=std    # Isi sesuai kebutuhan
  )
